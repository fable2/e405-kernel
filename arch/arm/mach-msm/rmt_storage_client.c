...
/* LGE_CHANGE_E [sunflwr.lee@lge.com] 20120113 */
{
	...
	msm_rpc_client_req2(client_nv, RMT_STORAGE_CHANGE_STATUS,
			NULL, NULL, NULL, NULL, -1);
}
// LGE_UPDATE_E

#if 1 /* jjm_debug */
extern int isPowerDown;
#endif
...
// LGE_UPDATE_S nv_default_write
	if(rmt_storage_work_queue != NULL)
	{
		//queue_work(rmt_storage_work_queue, &rmt_storage_work);
		queue_delayed_work(rmt_storage_work_queue, &rmt_storage_work,
						    msecs_to_jiffies(5000)); // 5 sec delay for Factory AT Command working delay
	}
// LGE_UPDATE_E
...
// LGE_UPDATE_S nv_default_write
	if(rmt_storage_work_queue != NULL)
	{
		destroy_workqueue(rmt_storage_work_queue);
		rmt_storage_work_queue = NULL;
	}
// LGE_UPDATE_E
...
// LGE_UPDATE_S nv_default_write
	rmt_storage_work_queue = create_singlethread_workqueue("rmt_storage_work");
	if (rmt_storage_work_queue == NULL) {
		ret = -ENOMEM;
		goto unreg_mdm_rpc;
	}
